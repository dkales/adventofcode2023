...
fn solve_stage2(input: &[Hailstone]) -> u64 {
    ...
    // eq1': eq1 & eq2:     x0*vy1-x0*vy0-x1*vy1+x1vy0 = y0*vx1-v0*vx0-y1*vx1+y1*vx0
    // eq2': eq1 & eq3:     x0*vz1-x0*vz0-x1*vz1+x1vz0 = z0*vx1-v0*vx0-z1*vx1+z1*vx0
    // eq3': eq4 & eq5:     x0*vy2-x0*vy0-x2*vy2+x2vy0 = y0*vx2-v0*vx0-y2*vx2+y2*vx0
    // eq4': eq4 & eq6:     x0*vz2-x0*vz0-x2*vz2+x2vz0 = z0*vx2-v0*vx0-z2*vx2+z2*vx0
    // eq5': eq7 & eq8:     x0*vy3-x0*vy0-x3*vy3+x3vy0 = y0*vx3-v0*vx0-y3*vx3+y3*vx0
    // eq6': eq7 & eq9:     x0*vz3-x0*vz0-x3*vz3+x3vz0 = z0*vx3-v0*vx0-z3*vx3+z3*vx0
    ...
    // eq3'': eq3' - eq5':  x0 * (vy2-vy3) - x2* (vy2-vy0) - x3 * (vy0-vy3) = y0 * (vx2-vx3) - y2 * (vx2-vx0) - y3 * (vx0-vx3)
    // eq4'': eq2' - eq4':  x0 * (vz1-vz2) - x1* (vz1-vz0) - x2 * (vz0-vz2) = z0 * (vx1-vx2) - z1 * (vx1-vx0) - z2 * (vx0-vx2)
    // eq5'': eq2' - eq6':  x0 * (vz1-vz3) - x1* (vz1-vz0) - x3 * (vz0-vz3) = z0 * (vx1-vx3) - z1 * (vx1-vx0) - z3 * (vx0-vx3)
    // eq6'': eq4' - eq6':  x0 * (vz2-vz3) - x2* (vz2-vz0) - x3 * (vz0-vz3) = z0 * (vx2-vx3) - z2 * (vx2-vx0) - z3 * (vx0-vx3)

    // make them a bit nicer, factoring out the variables
    // eq1'': x0 * (vy1-vy2) + vy0 * (x1-x2) - y0 * (vx1-vx2) + vx0 * (y2-y1) = -y1*vx1 + y2*vx2 + x1*vy1 - x2*vy2
    // eq2'': x0 * (vy1-vy3) + vy0 * (x1-x3) - y0 * (vx1-vx3) + vx0 * (y3-y1) = -y1*vx1 + y3*vx3 + x1*vy1 - x3*vy3
    // eq3'': x0 * (vy2-vy3) + vy0 * (x2-x3) - y0 * (vx2-vx3) + vx0 * (y3-y2) = -y2*vx2 + y3*vx3 + x2*vy2 - x3*vy3
    // eq4'': x0 * (vz1-vz2) + vz0 * (x1-x2) - z0 * (vx1-vx2) + vx0 * (z2-z1) = -z1*vx1 + z2*vx2 + x1*vz1 - x2*vz2
    // eq5'': x0 * (vz1-vz3) + vz0 * (x1-x3) - z0 * (vx1-vx3) + vx0 * (z3-z1) = -z1*vx1 + z3*vx3 + x1*vz1 - x3*vz3
    // eq6'': x0 * (vz2-vz3) + vz0 * (x2-x3) - z0 * (vx2-vx3) + vx0 * (z3-z2) = -z2*vx2 + z3*vx3 + x2*vz2 - x3*vz3
